...
class ImportSessionRequestSerializer(serializers.Serializer):
    ...
    source = serializers.ChoiceField(choices=WdicSource.choices, required=False, default=WdicSource.UNKNOWN)

class SessionListSerializer(serializers.ModelSerializer):
    handCount = serializers.IntegerField(read_only=True)
    class Meta:
        model = WdicSession
        fields = ["id", "name", "source", "started_at", "ended_at", "created_at", "handCount"]
...
class HandListSerializer(serializers.ModelSerializer):
    ...
    def get_board_cards_str(self, obj):
        ...
        if getattr(obj, 'flop_1', None): cards.append(obj.flop_1)
        if getattr(obj, 'flop_2', None): cards.append(obj.flop_2)
        if getattr(obj, 'flop_3', None): cards.append(obj.flop_3)
        if getattr(obj, 'turn', None):   cards.append(obj.turn)
...
class GuestRequiredAPIView(APIView):
    def get_guest(self, request) -> GuestIdentity:
        ...
        if not guest_header: raise serializers.ValidationError({"detail": "Missing X-Guest-Id"})
        try: guest_uuid = uuid.UUID(guest_header)
        except ValueError: raise serializers.ValidationError({"detail": "Invalid X-Guest-Id"})
...
class ImportSessionView(GuestRequiredAPIView):
    ...
    def post(self, request):
        ...
        if not hand_texts:
            return Response({"detail": "No hands found"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        with transaction.atomic():
            ...
            if not session:
                ...
                session = WdicSession.objects.create(guest=guest, name=name, source=source)
            else:
            ...
            for raw_hand in hand_texts:
                ...
                if meta.get("started_at_str"):
                    try:
                        started_at_dt = datetime.strptime(meta["started_at_str"], "%Y/%m/%d %H:%M:%S")
...
class SessionListView(GuestRequiredAPIView):
    def get(self, request):
        ...
        qs = WdicSession.objects.filter(guest=guest).annotate(handCount=Count("hands")).order_by("-created_at")[:limit]
