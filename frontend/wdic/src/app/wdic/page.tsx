...
import { getUserInfo, isLoggedIn, UserInfo } from "@/lib/auth.client";
...
function StatCard({ label, value, color, icon, subtext, user, progress, progressMax }: { label: string; value: string; color: string; icon: React.ReactNode; subtext?: string; user: any; progress?: number; progressMax?: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white/80 backdrop-blur-xl p-6 md:p-8 rounded-[2.5rem] md:rounded-[3rem] border border-white shadow-[0_15px_50px_rgba(0,0,0,0.02)] hover:shadow-[0_25px_60px_rgba(0,0,0,0.06)] transition-all duration-500 group relative overflow-hidden"
    >
      ...
      <div className="relative z-10">
        ...
        <div className={`text-3xl md:text-5xl font-[1000] tracking-tighter ${color} drop-shadow-sm leading-none`}>{value}</div>
        {subtext && (
...
export default function WdicSessionsPage() {
  ...
  return (
    <div className="flex-1 flex flex-col pt-24 md:pt-32">
      <div className="relative z-10 max-w-7xl mx-auto p-4 md:p-12 w-full pt-0">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-16 gap-12">
          <motion.div
            ...
            className="w-full"
          >
            <h1 className="text-4xl md:text-7xl lg:text-7xl font-[1000] tracking-tighter mb-4 drop-shadow-sm text-gray-900 uppercase leading-[0.9] w-full break-words">
